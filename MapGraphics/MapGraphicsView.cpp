...
void MapGraphicsView::handleChildMouseDoubleClick(QMouseEvent *event)
{
    ...
    MouseClick =_tileSource->qgs2ll(mousePoint,zoom);
...
void MapGraphicsView::ShowContextMenu(const QPoint &pos)
{
    //event->setAccepted(false);
    const QPointF  centerGeoPos = this->mapToScene(QPoint(this->width()/2,this->height()/2));
    QPointF mousePoint = _childView->mapToScene(_childView->mapFromGlobal(QCursor::pos()));
...
//   QAction action1("Установить точку посадки", this);
   QAction action2("Установить точку штатива", this);
//   QAction action3("Лететь сюда", this);
//   QAction action4("Точка посадки", this);
//   QAction action5("Точка взлета", this);
//   QAction action6("Точка 1", this);
//   QAction action7("Точка 2", this);
//   connect(&action1, SIGNAL(triggered()), this, SLOT(SetHomePosition()));
   connect(&action2, SIGNAL(triggered()), this, SLOT(SetComandPostPosition()));
//   connect(&action3, SIGNAL(triggered()), this, SLOT(MissionItem()));
//   connect(&action4, SIGNAL(triggered()), this, SLOT(MissionLanding()));
//   connect(&action5, SIGNAL(triggered()), this, SLOT(MissionTakeoff()));
//   connect(&action6, SIGNAL(triggered()), this, SLOT(MissionWP1()));
//   connect(&action7, SIGNAL(triggered()), this, SLOT(MissionWP2()));
...
void MapGraphicsView::SetHomePosition(void)
{
        HomePosition_Change = 1;
}

void MapGraphicsView::SetComandPostPosition(void)
{
        ComandPostPosition_Change = 1;
}

void MapGraphicsView::MissionItem(void)
{
        FlyTo_Change = 1;
}

void MapGraphicsView::MissionLanding(void)
{
        LANDING_Change = 1;
}

void MapGraphicsView::MissionTakeoff(void)
{
        TAKEOFF_Change = 1;
}

void MapGraphicsView::MissionWP1(void)
{
        WP1_Change = 1;
}

void MapGraphicsView::MissionWP2(void)
{
        WP2_Change = 1;
}
