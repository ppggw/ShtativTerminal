...
const QString MAPGRAPHICS_CACHE_FOLDER_NAME = "GTOSM";
const quint32 DEFAULT_CACHE_DAYS = 10;
...
//protected static
bool MapTileSource::cacheID2xyz(const QString & string, quint32 *y, quint32 *x, quint32 *z)
{
...
QImage *MapTileSource::fromDiskCache(const QString &cacheID)
{
    ...
    //
    const QString path2 = this->getDiskCacheFilePNG(x,y,z);
    QFile fp2(path2);
...

  {
      ...
      {
          qWarning() << "Failed to open" << QFileInfo(fp2.fileName()).baseName() << "from cache";
       //   return 0;
      }

      counter = 0;
          while (data2.length() < fp2.size())
          {
              data2 += fp2.read(20480);
              if (++counter >= MAX_DISK_CACHE_READ_ATTEMPTS)
              {
                  qWarning() << "Reading cache file" << fp.fileName() << "took too long. Aborting.";
       //           return 0;
        ...
        QImage  image5;
        QImage  image6;
      pixmap1.loadFromData(data2);
      pixmap2.loadFromData(data);


      image5=pixmap1.toImage();
      image6=pixmap2.toImage();
      ...
      QImage  NewImage ;
       ...
       // newImage = i mage;
         ...
         painter.drawImage(0,0, image5);
         painter.save();

         pixmap3 = QPixmap::fromImage(NewImage);





   QImage * image3 = new QImage();
   *image3 = pixmap3.toImage();
...
{
    QString toRet = this->getDiskCacheDirectory(x,y,z).absolutePath() % "/" % QString::number(x) % "." % this->tileFileExtension();
    return toRet;//(x,y,z)
}

QString MapTileSource::getDiskCacheFilePNG(quint32 x, quint32 y, quint8 z) const
{
    QString toRet = this->getDiskCacheDirectory(x,y,z).absolutePath() % "/" % QString::number(qPow(2,z)-1 - x) % ".png";
    return toRet;
}
